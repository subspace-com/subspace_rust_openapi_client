...
/// struct for typed errors of method [`web_rtc_cdn_service_get_web_rtc_cdn`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum WebRtcCdnServiceGetWebRtcCdnError {
...
pub async fn web_rtc_cdn_service_get_web_rtc_cdn(configuration: &configuration::Configuration, ) -> Result<crate::models::V1WebRtcCdnResponse, Error<WebRtcCdnServiceGetWebRtcCdnError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/v1/webrtc-cdn", local_var_configuration.base_path);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<WebRtcCdnServiceGetWebRtcCdnError> = serde_json::from_str(&local_var_content).ok();
